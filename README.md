...
- **Core engine and CLI** — Go code in `engine/`; build with `make build` → `bin/runfabric`. **Packages** (Node CLI/SDK, Python, Go, Java, .NET) live under `packages/`. See [docs/FILE_STRUCTURE.md](docs/FILE_STRUCTURE.md) and [docs/LAYOUT.md](docs/LAYOUT.md).
...
The **Node SDK** (`@runfabric/sdk`, `packages/node/sdk`) lets you use a single handler or mount RunFabric handlers in Express, Fastify, or Nest. The **Node CLI** is `@runfabric/cli` (`packages/node/cli`). See [docs/SDK_FRAMEWORKS.md](docs/SDK_FRAMEWORKS.md) and `packages/node/sdk/README.md` for `createHandler`, `mountExpress`, and `mountFastify`.
...
- `docs/HANDLER_SCENARIOS.md` — handler patterns
- `docs/SDK_FRAMEWORKS.md` — SDK and framework adapters
