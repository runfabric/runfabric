...
function stateBackendLines(stateBackend: StateBackend, stateNamespaceId: string): string[] {
  const stateKeyPrefix = `runfabric/${stateNamespaceId}/state`;
  ...
  if (stateBackend === "gcs") {
    return ["state:", "  backend: gcs", "  gcs:", "    bucket: ${env:RUNFABRIC_STATE_GCS_BUCKET}", `    prefix: ${stateKeyPrefix}`];
  }
  return ["state:", "  backend: azblob", "  azblob:", "    container: ${env:RUNFABRIC_STATE_AZBLOB_CONTAINER}", `    prefix: ${stateKeyPrefix}`];
}
...
export function buildConfigContent(
  ...
  return [
    ...
    `  - ${provider}`,
    "",
    ...stateBackendLines(stateBackend, stateNamespaceId),
    "",
...
function providerEnvExampleLines(credentialEnvVars: string[]): string[] {
  if (credentialEnvVars.length === 0) {
    return ["# no provider credential schema exposed for this provider"];
  }
  return credentialEnvVars.map((envName) => `${envName}=your-value`);
}

function stateBackendEnvExampleLines(stateBackend: StateBackend): string[] {
  if (stateBackend === "local") {
...
export function buildEnvExampleContent(credentialEnvVars: string[], stateBackend: StateBackend): string {
  return [
    "# Generated by runfabric init.",
    "# Copy this file to .env and replace placeholders.",
    "",
    "# Provider credentials",
    ...providerEnvExampleLines(credentialEnvVars),
    "",
    "# State backend credentials",
    ...stateBackendEnvExampleLines(stateBackend),
    ""
  ].join("\n");
}
...
export function buildProjectReadmeContent(params: ProjectReadmeContentParams): string {
  ...
  const envFileLines = [...providerEnvExampleLines(params.credentialEnvVars), "", ...stateBackendEnvExampleLines(params.stateBackend)];
