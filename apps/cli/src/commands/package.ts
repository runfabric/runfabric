...
function printPackageResult(
  payload: { function: string; artifacts: Array<{ provider: string; outputPath: string }> },
...
async function executePackageCommand(options: PackageCommandOptions): Promise<void> {
  ...
  const payload = { function: options.function || "all", artifacts: result.artifacts };
  printPackageResult(payload, options.json);
}
...
export const registerPackageCommand: CommandRegistrar = (program) => {
  program
    ...
    .action(async (options: PackageCommandOptions) => executePackageCommand(options));
};
