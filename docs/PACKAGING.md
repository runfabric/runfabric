...
| **Java** | *(same Go/engine binary)* | `@runfabric/sdk-java` (handler contract + HTTP adapter + framework adapters) |
...
- **Go / Java / .NET** (`packages/go/sdk`, `packages/java/sdk`, `packages/dotnet/sdk`): SDK-only packages; CLI is the Go binary built from `engine/cmd/runfabric`.
