...
`
	cfgPath := writeConfig(t, dir, cfg)
	root := NewRootCmd()
	root.SetArgs([]string{"logs", "-c", cfgPath, "--stage", "dev", "--all"})
	root.SetOut(&bytes.Buffer{})
	root.SetErr(&bytes.Buffer{})
	_ = root.Execute()
}
...
`
	cfgPath := writeConfig(t, dir, cfg)
	root := NewRootCmd()
	root.SetArgs([]string{"logs", "-c", cfgPath, "--stage", "dev"})
	root.SetOut(&bytes.Buffer{})
	root.SetErr(&bytes.Buffer{})
	if err := root.Execute(); err == nil {
...
`
	cfgPath := writeConfig(t, dir, cfg)
	root := NewRootCmd()
	root.SetArgs([]string{"metrics", "-c", cfgPath, "--stage", "dev", "--all"})
	root.SetOut(&bytes.Buffer{})
	root.SetErr(&bytes.Buffer{})
	_ = root.Execute()
}
...
`
	cfgPath := writeConfig(t, dir, cfg)
	root := NewRootCmd()
	root.SetArgs([]string{"traces", "-c", cfgPath, "--stage", "dev", "--provider", "aws-lambda", "--all"})
	root.SetOut(&bytes.Buffer{})
	root.SetErr(&bytes.Buffer{})
	_ = root.Execute()
}
...
`
	cfgPath := writeConfig(t, dir, cfg)
	root := NewRootCmd()
	root.SetArgs([]string{"invoke", "-c", cfgPath, "--stage", "dev", "--function", "api", "--payload", "{}"})
	root.SetOut(&bytes.Buffer{})
	root.SetErr(&bytes.Buffer{})
	_ = root.Execute()
}
...
func TestDev_HelpShowsStreamFromAndTunnelUrl(t *testing.T) {
	...
	root.SetErr(&bytes.Buffer{})
	if err := root.Execute(); err != nil {
		t.Fatalf("dev --help should succeed: %v", err)
	}
	b := out.Bytes()
	if !bytes.Contains(b, []byte("stream-from")) {
		t.Error("dev --help output should mention --stream-from")
	}
	if !bytes.Contains(b, []byte("tunnel-url")) {
		t.Error("dev --help output should mention --tunnel-url")
	}
}
