...
func runInit(o *initOpts) error {
	...
	initWrote(filepath.ToSlash(handlerPath))
	...
	initReady(projectDirLabel, o.Provider, o.Template, o.Lang, o.StateBackend)
	if projectDirLabel == "." {
		initNext("runfabric doctor --config runfabric.yml --stage dev")
	} else {
		initNext("cd " + projectDirLabel + " && runfabric doctor --config runfabric.yml --stage dev")
	}
	return nil
}
