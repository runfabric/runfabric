...
func Normalize(cfg *Config) {
	...
	// Functions: from FunctionsData (map or array) or from entry + triggers
	if cfg.Functions == nil {
		cfg.Functions = make(map[string]FunctionConfig)
	}
	if cfg.FunctionsData != nil {
		if len(cfg.FunctionsData.AsMap) > 0 {
			cfg.Functions = cfg.FunctionsData.AsMap
		} else if len(cfg.FunctionsData.AsArray) > 0 {
			for _, fo := range cfg.FunctionsData.AsArray {
