...
type Config struct {
	...
	// FunctionsData: unmarshals as either map (legacy) or array (reference). Normalize() sets Functions from it.
	FunctionsData *FunctionsRaw             `yaml:"functions,omitempty"`
	...
	// Layers: first-class layer declarations. Key = logical name; value = ARN or ref. Functions reference by name (e.g. layers: ["node-deps"]) and resolver expands to ARN.
...
// LayerConfig defines a named layer (e.g. for AWS Lambda). Arn is the full layer ARN; Name/Version are optional metadata.
...
// FunctionsRaw supports "functions" as either a map (legacy) or array (reference format). Normalize() converts to Config.Functions.
type FunctionsRaw struct {
	AsMap   map[string]FunctionConfig
