...
import (
	...
	internalproviders "github.com/runfabric/runfabric/engine/internal/extensions/providers"
...
func (p *DeployPlan) Execute(ctx context.Context) (*provider.DeployResult, error) {
	...
	receipt.Version = state.CurrentReceiptVersion
	receipt.UpdatedAt = time.Now().UTC().Format(time.RFC3339)
	state.EnrichReceiptWithAiWorkflow(receipt, p.cfg)

	if p.receipts != nil {
		if err := p.receipts.Save(receipt); err != nil {
			return nil, err
		}
	} else {
