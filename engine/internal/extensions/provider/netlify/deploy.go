...
func (Runner) Deploy(ctx context.Context, cfg *config.Config, stage, root string) (*providers.DeployResult, error) {
	...
	if siteID == "" {
		...
		if err := apiutil.APIPost(ctx, netlifyAPI+"/sites", "NETLIFY_AUTH_TOKEN", map[string]string{"name": cfg.Service + "-" + stage}, &createResp); err != nil {
	...
	result := apiutil.BuildDeployResult("netlify", cfg, stage)
