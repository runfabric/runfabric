...
func New(opts Options) (*Boundary, error) {
	b := &Boundary{
		providers:  extensions.NewBuiltinProviderRegistry(),
...
func (b *Boundary) ResolveProvider(name string) (providers.Provider, error) {
	return b.providers.Get(strings.TrimSpace(name))
}
...
func (b *Boundary) RefreshExternal() error {
	...
	for _, m := range res.Plugins {
		...
		if _, err := b.providers.Get(m.ID); err == nil && !b.discoverOptions.PreferExternal {
			continue
		}
		b.providers.Register(external.NewExternalProviderAdapter(m.ID, m.Executable))
	}
	return nil
}
