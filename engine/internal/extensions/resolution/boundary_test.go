...
func TestNewBoundary_RegistersBuiltinAndAPIProviders(t *testing.T) {
	...
	for _, name := range []string{"aws", "aws-lambda", "gcp-functions", "vercel", "netlify"} {
...
func TestIsInternalProvider_AWSOnly(t *testing.T) {
	...
	if !b.IsInternalProvider("aws") || !b.IsInternalProvider("aws-lambda") {
		t.Fatal("expected aws and aws-lambda to be internal providers")
	}
...
func TestIsAPIDispatchProvider(t *testing.T) {
	...
	if b.IsAPIDispatchProvider("aws") {
		t.Fatal("expected aws to be non-API-dispatched")
	}
}
...
func TestNewBoundary_PreferExternalHonorsVersionPinAndKeepsAWSInternal(t *testing.T) {
	...
	writeExternalProvider(t, home, "aws", "9.9.9")
	...
	aws := b.PluginRegistry().Get("aws")
	if aws == nil {
		t.Fatal("expected aws plugin manifest")
	}
	if aws.Source == "external" {
		t.Fatal("aws must remain internal even when preferExternal=true")
	}
}
