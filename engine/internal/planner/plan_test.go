...
func TestBuildPlan_EmptyFunctions(t *testing.T) {
	...
	if plan.Provider != "aws" || plan.Service != "svc" || plan.Stage != "dev" {
...
func TestBuildPlan_OneFunctionNoEvents(t *testing.T) {
	...
	plan := BuildPlan(cfg, "dev")
	// build:api + lambda:api
	if len(plan.Actions) != 2 {
		t.Fatalf("expected 2 actions (build+lambda), got %d", len(plan.Actions))
	}
	...
	if plan.Actions[1].ID != "lambda:api" || len(plan.Actions[1].DependsOn) != 1 || plan.Actions[1].DependsOn[0] != "build:api" {
...
func TestBuildPlan_OneFunctionHTTPAndCron(t *testing.T) {
	...
	plan := BuildPlan(cfg, "dev")
	// build:api, lambda:api, httpapi:service, http:api:0, schedule:api:1
...
func TestBuildPlan_TwoFunctions(t *testing.T) {
	...
	plan := BuildPlan(cfg, "dev")
	// build:api, lambda:api, build:worker, lambda:worker = 4
	...
	for _, id := range []string{"build:api", "lambda:api", "build:worker", "lambda:worker"} {
		if !ids[id] {
