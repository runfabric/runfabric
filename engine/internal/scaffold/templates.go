// Package scaffold provides shared logic for generating handler content and
// function config entries. Used by init and generate.
package scaffold
...
// BuildFunctionEntry returns a map suitable for configpatch.AddFunction: handler path,
// memory, timeout, and events. Route is for HTTP (e.g. "GET:/hello"), schedule for cron,
// queueName for queue. Method and path are parsed from route (METHOD:PATH); default GET and /<name>.
func BuildFunctionEntry(handlerPath, trigger, route, schedule, queueName string) map[string]any {
	...
	case planner.TriggerCron:
		...
		events = append(events, map[string]any{"cron": s})
	case planner.TriggerQueue:
		q := queueName
		if q == "" {
			q = "my-queue"
