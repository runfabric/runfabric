...
func discoverActualState(ctx context.Context, clients *AWSClients, cfg *config.Config, stage string) (*planner.ActualState, error) {
	...
	for pager.HasMorePages() {
		...
		for _, fn := range page.Functions {
			...
			state.Lambdas = append(state.Lambdas, item)
		}
	}
...
func desiredStateFromConfig(cfg *config.Config, stage string, artifacts map[string]providers.Artifact) *planner.DesiredState {
	...
	for fnName, fn := range cfg.Functions {
		...
		desired.Lambdas = append(desired.Lambdas, planner.DesiredLambda{
...
func actualLambdaMap(actual *planner.ActualState) map[string]planner.ActualLambda {
	out := map[string]planner.ActualLambda{}
	if actual == nil {
		return out
	}
...
func actualIntegrationMap(actual *planner.ActualState) map[string]planner.ActualIntegration {
	out := map[string]planner.ActualIntegration{}
	if actual == nil {
		return out
	}
	for _, i := range actual.Integrations {
		out[i.ID] = i
	}
	return out
}
