...
func (Invoker) Invoke(ctx context.Context, cfg *config.Config, stage, function string, payload []byte, receipt *state.Receipt) (*providers.InvokeResult, error) {
	...
	if resp.StatusCode >= 400 {
		return &providers.InvokeResult{Provider: "fly-machines", Function: function, Output: fmt.Sprintf("HTTP %d: %s", resp.StatusCode, out)}, nil
	}
	return &providers.InvokeResult{Provider: "fly-machines", Function: function, Output: out}, nil
}
