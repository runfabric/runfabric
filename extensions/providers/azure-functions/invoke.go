...
// Invoker invokes via HTTP POST to the function app URL.
type Invoker struct{}
...
func (Invoker) Invoke(ctx context.Context, cfg sdkprovider.Config, stage, function string, payload []byte, receipt any) (*sdkprovider.InvokeResult, error) {
	...
	if resp.StatusCode >= 400 {
		return &sdkprovider.InvokeResult{Provider: "azure-functions", Function: function, Output: fmt.Sprintf("HTTP %d: %s", resp.StatusCode, out)}, nil
	}
	return &sdkprovider.InvokeResult{Provider: "azure-functions", Function: function, Output: out}, nil
}
