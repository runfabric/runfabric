...
func TestLoggerLogs_UsesWranglerTailWhenAvailable(t *testing.T) {
	...
	result, err := logger.Logs(context.Background(), cfg, "dev", "api", sdkprovider.ReceiptView{Metadata: map[string]string{"worker": "demo-dev"}})
	if err != nil {
...
func TestLoggerLogs_FallsBackToAPITailWhenWranglerUnavailable(t *testing.T) {
	...
	oldClient := sdkprovider.DefaultClient
	sdkprovider.DefaultClient = ts.Client()
	defer func() { sdkprovider.DefaultClient = oldClient }()
