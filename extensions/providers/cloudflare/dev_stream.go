...
func RedirectToTunnel(ctx context.Context, cfg sdkprovider.Config, stage, tunnelURL string) (*DevStreamState, error) {
	...
	zoneID := sdkprovider.Env("CLOUDFLARE_ZONE_ID")
	...
	state.StatusMessage = "full route rewrite available when Cloudflare prerequisites are configured"
	...
	if len(state.MissingPrereqs) > 0 || accountID == "" || token == "" {
		...
		if accountID == "" {
			state.MissingPrereqs = append(state.MissingPrereqs, "CLOUDFLARE_ACCOUNT_ID")
		}
		if token == "" {
			state.MissingPrereqs = append(state.MissingPrereqs, "CLOUDFLARE_API_TOKEN")
		}
		if zoneID == "" {
			state.MissingPrereqs = append(state.MissingPrereqs, "CLOUDFLARE_ZONE_ID")
		}
		state.StatusMessage = "lifecycle-only fallback: Cloudflare route rewrite prerequisites are not fully configured"
		return state, nil
	}
...
func (s *DevStreamState) Restore(ctx context.Context, accountID string) error {
	...
	if s.ZoneID == "" {
		s.ZoneID = sdkprovider.Env("CLOUDFLARE_ZONE_ID")
	}
	if accountID == "" {
		accountID = s.AccountID
	}
	if accountID == "" || sdkprovider.Env("CLOUDFLARE_API_TOKEN") == "" {
		return nil
	}
	var errs []error
...
func resolveDevRoutePattern(cfg sdkprovider.Config, stage string) string {
	if v := strings.TrimSpace(sdkprovider.Env("CLOUDFLARE_DEV_ROUTE_PATTERN")); v != "" {
		return v
	}
	if cfg == nil {
		return ""
