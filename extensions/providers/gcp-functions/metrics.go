...
func FetchMetrics(ctx context.Context, cfg sdkprovider.Config, stage string) (map[string]FunctionMetrics, error) {
	...
	for fnName := range sdkprovider.Functions(cfg) {
		funcID := fmt.Sprintf("%s-%s-%s", service, stage, fnName)
...
func gcpListEntriesOnce(ctx context.Context, project, filter string, pageSize int) ([]gcpLogEntry, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, gcpHost(gcpLoggingEntriesAPI), strings.NewReader(string(bodyBytes)))
	if err != nil {
		return nil, err
	}
