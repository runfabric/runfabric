...
// This implementation uses a best-effort strategy and falls back to lifecycle-only mode when prerequisites are missing.
// Full route rewrite at provider edge would require one of:
...
func RedirectToTunnel(ctx context.Context, cfg sdkprovider.Config, stage, tunnelURL string) (*DevStreamState, error) {
	...
	state := &DevStreamState{
		...
		GatewaySetURL:     strings.TrimSpace(sdkprovider.Env("GCP_DEV_STREAM_GATEWAY_SET_URL")),
		GatewayRestoreURL: strings.TrimSpace(sdkprovider.Env("GCP_DEV_STREAM_GATEWAY_RESTORE_URL")),
	}
	state.EffectiveMode = "lifecycle-only"
	state.StatusMessage = "lifecycle-only fallback: provider route rewrite hooks are not fully configured"
	...
	if state.GatewaySetURL != "" && state.GatewayRestoreURL != "" {
		...
		if err := sdkprovider.APIPost(ctx, state.GatewaySetURL, "GCP_ACCESS_TOKEN", payload, nil); err != nil {
			state.EffectiveMode = "lifecycle-only"
		...
		state.Applied = true
		state.EffectiveMode = "route-rewrite"
	...
	getResp, err := sdkprovider.DefaultClient.Do(getReq)
	if err != nil {
		state.EffectiveMode = "lifecycle-only"
	...
	patchResp, err := sdkprovider.DefaultClient.Do(patchReq)
	if err != nil {
		state.EffectiveMode = "lifecycle-only"
	...
	state.Applied = true
	state.EffectiveMode = "conditional-mutation"
...
func (s *DevStreamState) Restore(ctx context.Context, region string) error {
	...
	if s.GatewayApplied {
		...
		if err := sdkprovider.APIPost(ctx, s.GatewayRestoreURL, "GCP_ACCESS_TOKEN", payload, nil); err != nil {
