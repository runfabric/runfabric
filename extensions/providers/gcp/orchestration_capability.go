...
func (Runner) SyncOrchestrations(ctx context.Context, req sdkprovider.OrchestrationSyncRequest) (*sdkprovider.OrchestrationSyncResult, error) {
	...
	project := strings.TrimSpace(sdkprovider.Env("GCP_PROJECT"))
	if project == "" {
		project = strings.TrimSpace(sdkprovider.Env("GCP_PROJECT_ID"))
	}
	if project == "" {
	...
	for _, decl := range decls {
		...
		if patchResp.StatusCode == http.StatusNotFound {
			...
			createResp, err := sdkprovider.DefaultClient.Do(createReq)
			if err != nil {
				return nil, err
			}
