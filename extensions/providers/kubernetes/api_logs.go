...
func (Logger) Logs(ctx context.Context, cfg sdkprovider.Config, stage, function string, receipt any) (*sdkprovider.LogsResult, error) {
	...
	if err != nil {
		return &sdkprovider.LogsResult{
			Provider: "kubernetes",
			Function: function,
			Lines:    []string{"kubeconfig: " + err.Error() + " (use: kubectl logs -n " + namespace + " -l app=" + service + " --tail=100)"},
		}, nil
	}
	...
	pods, err := clientset.CoreV1().Pods(namespace).List(ctx, metav1.ListOptions{LabelSelector: "app=" + service})
	if err != nil {
		return &sdkprovider.LogsResult{Provider: "kubernetes", Function: function, Lines: []string{"list pods: " + err.Error()}}, nil
	}
	if len(pods.Items) == 0 {
		return &sdkprovider.LogsResult{Provider: "kubernetes", Function: function, Lines: []string{"No pods found for app=" + service + " in namespace " + namespace}}, nil
	}
	pod := pods.Items[0]
	containerName := "app"
