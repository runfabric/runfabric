...
func newLoginCmd(opts *common.GlobalOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			common.StatusRunning(opts.JSONOutput, "Requesting device code...")
			dc, err := requestDeviceCode(client, resolvedAuthURL, clientID, scope)
			if err != nil {
				common.StatusFail(opts.JSONOutput, "Login failed.")
				return err
			}
			...
			common.StatusRunning(opts.JSONOutput, "Waiting for authorization...")
			...
			common.StatusDone(opts.JSONOutput, "Login complete.")
			if opts.JSONOutput {
				return common.WriteJSONEnvelope(c.OutOrStdout(), true, "login", map[string]any{
					"authUrl": resolvedAuthURL,
					"tokenId": record.ID,
					"user":    me,
				}, nil)
...
func newWhoAmICmd(opts *common.GlobalOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if opts.JSONOutput {
				return common.WriteJSONEnvelope(c.OutOrStdout(), true, "whoami", map[string]any{"user": me}, nil)
			}
			subject := firstString(me, "sub", "id", "user_id")
			if subject == "" {
				subject = nestedString(me, "claims", "sub")
			}
			if subject == "" {
				subject = strings.TrimSpace(active.Subject)
			}
			printed := false
			if subject != "" {
			...
			email := firstString(me, "email")
			if email == "" {
				email = nestedString(me, "user", "email")
			}
			if email == "" {
			...
			name := firstString(me, "name", "username", "displayName")
			if name == "" {
				name = nestedString(me, "user", "displayName")
			}
			if name == "" {
				name = nestedString(me, "user", "name")
			}
			if name == "" {
				name = nestedString(me, "user", "username")
			}
			if name == "" {
...
func newLogoutCmd(opts *common.GlobalOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if active == nil {
				if opts.JSONOutput {
					return common.WriteJSONEnvelope(c.OutOrStdout(), true, "logout", map[string]any{"cleared": 0}, nil)
				}
			...
			if opts.JSONOutput {
				return common.WriteJSONEnvelope(c.OutOrStdout(), true, "logout", map[string]any{"cleared": cleared}, nil)
			}
...
func newTokenListCmd(opts *common.GlobalOptions) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if opts.JSONOutput {
				return common.WriteJSONEnvelope(c.OutOrStdout(), true, "token list", map[string]any{"tokens": out}, nil)
			}
			if len(out) == 0 {
...
func newTokenRevokeCmd(opts *common.GlobalOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if opts.JSONOutput {
				return common.WriteJSONEnvelope(c.OutOrStdout(), true, "token revoke", map[string]any{"revoked": revoked}, nil)
			}
