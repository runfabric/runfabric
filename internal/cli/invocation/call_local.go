...
func newCallLocalCmd(opts *GlobalOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			common.StatusRunning(opts.JSONOutput, "Starting local server...")
			...
			if serve && watch {
				...
				for {
					...
					if err != nil {
						common.StatusFail(opts.JSONOutput, "Call-local failed.")
						return common.PrintFailure("call-local", err)
					}
					select {
					case <-watchChan:
						if !opts.JSONOutput {
			...
			common.StatusDone(opts.JSONOutput, "Local server ready.")
			_ = provider
			_ = method
			_ = path
			_ = query
			...
			_ = entry
			if opts.JSONOutput {
				return common.PrintJSONSuccess("call-local", result)
			}
			return common.PrintSuccess("call-local", result)
		},
	}
