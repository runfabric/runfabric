...
` + "```text\n$RUNFABRIC_HOME/plugins/providers/%s/0.1.0/\n```" + `
...
- Command resolution order is ` + "`config.commands.*`" + ` -> ` + "`<operation>Command`" + ` -> env var.
...
Configure explicit deploy/remove/invoke/log command wiring using ` + "`config.commands.*`" + ` or the ` + "`*_CMD`" + ` environment variables for your provider.
`
	return strings.NewReplacer(
		"__PROVIDER_ID__", providerID,
		"__PROVIDER_ENV_PREFIX__", providerEnvPrefix,
