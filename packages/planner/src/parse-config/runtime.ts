...
function parseRuntimeModeValue(
  ...
  path: string,
  errors: string[]
): "native-compat" | "engine" | undefined {
  const normalized = normalizeRuntimeMode(value);
  if (!normalized) {
    errors.push(`${path} must be one of: ${runtimeModeList()}`);
    return undefined;
  }
  return normalized;
}
...
export function readOptionalRuntimeModeAtPath(
  value: unknown,
  path: string,
  errors: string[]
): "native-compat" | "engine" | undefined {
  ...
  return parseRuntimeModeValue(value, path, errors);
}
