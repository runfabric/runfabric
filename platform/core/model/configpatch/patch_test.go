...
`
	...
	entry := map[string]any{
		"entry":    "src/hello.handler",
		"triggers": []any{map[string]any{"type": "http", "method": "GET", "path": "/hello"}},
	}
	...
	if !strings.Contains(content, "name: hello") {
		t.Errorf("expected functions array entry for hello in output, got:\n%s", content)
	}
	if !strings.Contains(content, "entry: src/hello.handler") {
...
`
	...
	entry := map[string]any{
		"entry":    "src/newfn.handler",
		"triggers": []any{map[string]any{"type": "cron", "schedule": "rate(5 minutes)"}},
	}
	...
	content := string(data)
	if !strings.Contains(content, "name: existing") || !strings.Contains(content, "name: newfn") {
...
`
	...
	entry := map[string]any{"entry": "src/hello.handler"}
