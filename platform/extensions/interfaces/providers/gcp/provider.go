...
import (
	...
	gcptarget "github.com/runfabric/runfabric/platform/extensions/internal/providers/gcp"
)
...
func FetchMetrics(ctx context.Context, cfg *coreconfig.Config, stage string) (map[string]FunctionMetrics, error) {
	...
	metrics, err := gcptarget.FetchMetrics(ctx, engCfg, stage)
	if err != nil {
		return nil, err
	}
...
func FetchTraces(ctx context.Context, cfg *coreconfig.Config, stage string) ([]TraceSummary, error) {
	...
	traces, err := gcptarget.FetchTraces(ctx, engCfg, stage)
	if err != nil {
		return nil, err
	}
