...
func (Runner) Deploy(ctx context.Context, cfg sdkprovider.Config, stage, root string) (*sdkprovider.DeployResult, error) {
	...
	if b, _ := os.ReadFile(scriptPath); len(b) == 0 && len(coreCfg.Functions) > 0 {
		for _, fn := range coreCfg.Functions {
	...
	result := apiutil.BuildSDKDeployResult("cloudflare-workers", cfg, stage)
