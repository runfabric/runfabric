...
func (Runner) Deploy(ctx context.Context, cfg sdkprovider.Config, stage, root string) (*sdkprovider.DeployResult, error) {
	...
	_ = coreCfg
	if apiutil.Env("NETLIFY_AUTH_TOKEN") == "" {
		return nil, fmt.Errorf("NETLIFY_AUTH_TOKEN is required")
	}
	siteID := apiutil.Env("NETLIFY_SITE_ID")
	if siteID == "" {
		var createResp struct {
			ID string `json:"id"`
		}
		if err := apiutil.APIPost(ctx, netlifyAPI+"/sites", "NETLIFY_AUTH_TOKEN", map[string]string{"name": coreCfg.Service + "-" + stage}, &createResp); err != nil {
	...
	result := apiutil.BuildSDKDeployResult("netlify", cfg, stage)
