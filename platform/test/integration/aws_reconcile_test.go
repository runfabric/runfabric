...
`
	...
	planResult, err := p.Plan(context.Background(), provider.PlanRequest{Config: cfg2, Stage: "dev", Root: cfgContent2})
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = p.Deploy(context.Background(), provider.DeployRequest{Config: cfg2, Stage: "dev", Root: tmp})
	if err != nil {
		t.Fatal(err)
	}

	_, err = p.Remove(context.Background(), provider.RemoveRequest{Config: cfg2, Stage: "dev", Root: tmp})
	if err != nil {
		t.Fatal(err)
	}
}
