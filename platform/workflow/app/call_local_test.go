...
func TestResolveSimulatorForLocal_DefaultLocal(t *testing.T) {
	...
	ctx := &AppContext{Config: &config.Config{}, Extensions: newExtensionsConnectorFromBoundary(b)}

	simID := resolveSimulatorIDForLocal(ctx)
	if simID != "local" {
		t.Fatalf("simulator id=%q want local", simID)
	}
}
...
func TestLocalInvokeHandler_UsesSimulatorPlugin(t *testing.T) {
	...
	ctx := &AppContext{
		...
		Stage:      "dev",
		Extensions: newExtensionsConnectorFromBoundary(b),
	}

	simID := resolveSimulatorIDForLocal(ctx)
	h := newLocalInvokeHandler(ctx, simID, "api")
