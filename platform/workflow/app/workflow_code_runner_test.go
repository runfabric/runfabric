...
func TestCodeRunnerFallsBackWithoutFunction(t *testing.T) {
	r := &invokeCodeStepRunner{invoke: func(string, []byte) (any, error) {
		t.Fatal("invoke must not be called without input.function")
		return nil, nil
	}}
	out := map[string]any{}
	res, err := r.ExecuteStep(nil, codeStep(nil), out, map[string]any{})
	if err != nil {
...
func TestCodeRunnerInvokesFunctionWithPayload(t *testing.T) {
	...
	r := &invokeCodeStepRunner{invoke: func(function string, payload []byte) (any, error) {
		gotFunction, gotPayload = function, string(payload)
		return map[string]any{"ok": true}, nil
	}}

	out := map[string]any{}
	res, err := r.ExecuteStep(nil, codeStep(map[string]any{
		"function": "api",
...
func TestCodeRunnerDefaultsPayloadToEmptyObject(t *testing.T) {
	...
	r := &invokeCodeStepRunner{invoke: func(_ string, payload []byte) (any, error) {
		gotPayload = string(payload)
		return "ok", nil
	}}
	if _, err := r.ExecuteStep(nil, codeStep(map[string]any{"function": "api"}), map[string]any{}, map[string]any{}); err != nil {
		t.Fatalf("ExecuteStep: %v", err)
	}
	if gotPayload != "{}" {
...
func TestCodeRunnerPropagatesInvokeError(t *testing.T) {
	...
	_, err := r.ExecuteStep(nil, codeStep(map[string]any{"function": "api"}), map[string]any{}, map[string]any{})
