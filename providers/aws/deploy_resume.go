...
import (
	...
	"github.com/runfabric/runfabric/internal/runtime/build"
	"github.com/runfabric/runfabric/providers/aws/triggers"
)
...
func ResumeDeploy(
	...
	receipt := &state.Receipt{
		...
		Stage:        stage,
		Provider:     "aws",
		DeploymentID: fmt.Sprintf("aws-%s-%d", stage, time.Now().Unix()),
...
type ResumeDependencies struct {
	...
	ActualMap     map[string]planner.ActualLambda
	HTTPAPI       *HTTPAPI
	...
	phases := []deployexec.Phase{
		...
		deployexec.PhaseFunc{
			...
			Fn: func(ctx context.Context, ectx *deployexec.Context) error {
				...
				deps.ActualMap = actualLambdaMap(actual)
				return nil
			},
		},
		deployexec.PhaseFunc{
		...
		deployexec.PhaseFunc{
			...
			Fn: func(ctx context.Context, ectx *deployexec.Context) error {
				for fnName, fn := range cfg.Functions {
					...
					lambdaPhysicalName := functionName(cfg, stage, fnName)

					var actualLambdaPtr *planner.ActualLambda
					if actualLambda, ok := deps.ActualMap[lambdaPhysicalName]; ok {
						actualLambdaPtr = &actualLambda
					}

					desiredLambda := planner.DesiredLambda{
						Name:            lambdaPhysicalName,
						Runtime:         fn.Runtime,
						Handler:         fn.Handler,
					...
					changeSet := detectFunctionChange(fnName, artifact, desiredLambda, actualLambdaPtr, deps.ReceiptMap)
					...
					ectx.Metadata["lambda:"+fnName+":arn"] = deployed.FunctionArn
				...
				for fnName, fn := range cfg.Functions {
					...
					for i, ev := range fn.Events {
						...
						if err := ensureAPIGatewayInvokePermission(
							ctx, deps.Clients, cfg, stage, fnName,
		...
		deployexec.PhaseFunc{
			...
			Fn: func(ctx context.Context, ectx *deployexec.Context) error {
				...
				cronClients := &triggers.CronClients{
				...
				if err := triggers.EnsureCronRules(ctx, cronClients, lambdaNameByFunction, lambdaARNByFunction, cfg, stage); err != nil {
					return err
				}
				queueClients := &triggers.QueueClients{SQS: deps.Clients.SQS, Lambda: deps.Clients.Lambda}
				if err := triggers.EnsureQueueTriggers(ctx, queueClients, lambdaNameByFunction, lambdaARNByFunction, cfg, stage); err != nil {
					return err
				}
				storageClients := &triggers.StorageClients{S3: deps.Clients.S3, Lambda: deps.Clients.Lambda}
				if err := triggers.EnsureStorageTriggers(ctx, storageClients, lambdaARNByFunction, cfg, stage); err != nil {
					return err
				}
				ebClients := &triggers.EventBridgeClients{
					EventBridge: deps.Clients.EventBridge,
					Lambda:      deps.Clients.Lambda,
					Region:      deps.Clients.AWS.Region,
					AccountID:   deps.Clients.AccountID,
				}
				if err := triggers.EnsureEventBridgeRules(ctx, ebClients, lambdaARNByFunction, cfg, stage); err != nil {
					return err
				}
				return nil
			},
